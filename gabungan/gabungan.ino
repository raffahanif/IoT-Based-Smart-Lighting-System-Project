...
void loop() {
  ...
  if (modeSekarang == OTOMATIS) {
    ...
    int nilai = bacaSensor();
    Serial.printf("Sensor: %d\n", nilai);
    Blynk.virtualWrite(V0, nilai);
    int pwm = 0;
    int statusLampu = 0;

    if (nilai >= batasGelap) {            // >= supaya 1023 masuk
      pwm = 1023;                          // pastikan maksimal
      statusLampu = 2;
} 
    else if (nilai > batasTerang) {
      pwm = map(nilai, batasTerang, batasGelap-1, 0, 900);  // kurangi 1 agar map valid
      statusLampu = 1;
} 
    else {
      pwm = 0;
